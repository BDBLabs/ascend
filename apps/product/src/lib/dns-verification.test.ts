...
describe('hasVerificationRecord', () => {
  ...
  it('accepts the exact record, including one split into chunks', async () => {
    expect(await hasVerificationRecord('example.com', token, async () => [[`jbox-verify=${token}`]])).toBe(true);
    expect(await hasVerificationRecord('example.com', token, async () => [['jbox-verify=', token]])).toBe(true);
  });

  it('queries the _jbox-verify label', async () => {
    const names: string[] = [];
  ...
  it('refuses a different token, other records, and DNS errors', async () => {
    expect(await hasVerificationRecord('example.com', token, async () => [[`jbox-verify=${'b'.repeat(32)}`]])).toBe(false);
